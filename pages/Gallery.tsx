    ...
    <div className="flex-grow bg-[#F8F9FA]">
            ...
            {categories.map(cat => (
              <button 
                ...
                className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 ${
                  activeFilter === cat 
                  ? "bg-primary text-[#0d1b12] shadow-md scale-105" 
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[300px]">
          {filteredItems.map((item, index) => (
            <div 
              key={item.id} 
              className={`group relative rounded-2xl overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gray-200 ${
                // Make some items span 2 rows for masonry effect if desired, 
                // keeping it simple for now but adding class logic strictly
                index % 3 === 0 ? 'lg:col-span-1 lg:row-span-1' : '' 
              }`}
      ...
      <section className="bg-white py-16 border-t border-gray-100">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <button className="bg-primary text-[#0d1b12] font-bold py-4 px-8 rounded-xl shadow-lg hover:bg-primary-hover hover:shadow-xl transition-all hover:-translate-y-0.5 text-lg">
          ...
          >
            <div className="relative w-full h-full rounded-lg overflow-hidden shadow-2xl">
                {selectedImage.type === 'video' ? (
                   // Simulating video player for demo
